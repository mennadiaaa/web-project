...
// If not logged in or not an organizer, kick them out
...
async function getEvents() {
    try {
        const response = await fetch(`${API}?organizerId=${loggedUser.id}`);
...
async function handleSubmit(e) {
    ...
    if (eventDateTime <= new Date()) {
        showToast("Event must be in the future");
        return;
    }
    ...
    try {
        ...
        formData.append("organizerId", loggedUser.id);
        ...
        showToast(method === "POST" ? "Event Added" : "Event Updated");
        await renderEvents();
        resetForm();
